describe('Смоук сайта WEBDRIVER.IO, тест №2', () => {
  ...
  it('Проверить, что заголовок = Introduction', async () => {
    ...
    await browser.waitUntil(async () => (await header.getText()) === 'Introduction', {
      timeout: 5000,
  ...
  it('Проверка отображения X-ссылки до скролла', async () => {
    ...
    console.log('Before scroll isDisplayed:', await xLink.isDisplayed());
  ...
  it('Проверка отображения X-ссылки после скролла', async () => {
    ...
    console.log('After scroll isDisplayed:', await xLink.isDisplayed());
